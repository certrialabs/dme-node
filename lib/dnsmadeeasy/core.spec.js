...
describe('Core', function() {
  ...
  describe('GET', function() {
    it('sends a request to the correct URI', async function() {
      var headers = {
        test: 'test'
      };
  ...
  describe('PUT', function() {
    it('sends a request to the correct URI', async function() {
      ...
      var response = await client.put(['test'], payload);
      expect(response).to.be.eql({
        status: 'ok'
      });
    });
  ...
  describe('POST', function() {
    it('sends a request to the correct URI', async function() {
      ...
      var response = await client.post(['test'], payload);
      expect(response).to.be.eql({
        status: 'ok'
      });
    });
  ...
  describe('DELETE', function() {
    it('sends a request to the correct URI', async function() {
      ...
      var response = await client.delete(['test']);
      expect(response).to.be.eql({
        status: 'ok'
      });
    });
  ...
  describe('#utils', function() {
    describe('_generateHeaders', function() {
      it('gets generates proper headers', function() {
        ...
        this.sandbox.stub(Date.prototype, 'toGMTString').callsFake(function() {
          return 'abc';
        });
    ...
    describe('_sendRequest', function() {
      it('sends out a request and returns with a promise', async function() {
        nock('http://test.com')
          .get('/test')
          .reply(200, {
            status: 'ok'
        ...
        var response = await client._sendRequest(requestParams);
        expect(response).to.be.eql({
          status: 'ok'
        });
      });

      it('sends out a request and returns with an error', async function() {
        nock('http://test.com')
          .get('/test')
        ...
        try {
          await client._sendRequest(requestParams);
        } catch (ex) {
